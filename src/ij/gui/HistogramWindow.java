...
public class HistogramWindow extends ImageWindow implements Measurements, ActionListener, 
	...
	void drawPlot(long maxCount, ImageProcessor ip) {
		...
		histogramLength("normal",  maxCount, ip);
	}

	void drawLogPlot (long maxCount, ImageProcessor ip) {
		double max = Math.log(maxCount);
		ip.setColor(Color.gray);
		histogramLength("log",  (long) max, ip);
		ip.setColor(Color.black);
	}

	void histogramLength(String type, long maxCount, ImageProcessor ip) {
		...
		if (histogram.length==256) {
			...
			for (int i = 0; i < 256; i++) {
				int x =(int)(i*scale2);
				int y = getYLogOrNormal(type, histogram[i], maxCount);
			...
			for (int i=0; i<HIST_WIDTH; i++) {
				...
				y = getYLogOrNormal(type, histogram[index], maxCount);
				if (y>HIST_HEIGHT) y = HIST_HEIGHT;
			...
			for (int i=0; i<histogram.length; i++) {
				...
				if (value>0L) {
					int y = getYLogOrNormal(type, value, maxCount);
					if (y>HIST_HEIGHT) y = HIST_HEIGHT;
	...
	int getYLogOrNormal(String type, long value, long maxCount){
		if (type == "log") {
			return value==0?0:(int)(HIST_HEIGHT*Math.log(value)/maxCount);
		}
		return (int)(((double)HIST_HEIGHT*(double)value)/maxCount);
	}
